...
<?= form_open_multipart($car ? 'admin/cars/edit/'.$car->id : 'admin/cars/add') ?>
    <div class="row">
        <div class="col-md-6">
            <div class="mb-3">
                ...
                <select name="brand_id" class="form-select" id="brandId" required>
                    ...
                    <option value="<?= $b->id ?>" <?= ($car && $car->brand_id==$b->id) ? 'selected' : '' ?>>
                        <?= html_escape($b->name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <select name="model_id" class="form-select" id="modelId" required>
                    ...
                    <option value="<?= $m->id ?>" <?= ($car && $car->model_id==$m->id) ? 'selected' : '' ?>>
                        <?= html_escape($m->name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <input type="text" name="variant" class="form-control"
                    value="<?= $car ? html_escape($car->variant) : '' ?>">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select name="fuel_type_id" class="form-select" required>
                    ...
                    <option value="<?= $f->id ?>" <?= ($car && $car->fuel_type_id==$f->id) ? 'selected' : '' ?>>
                        <?= $f->name ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <select name="transmission_id" class="form-select" required>
                    ...
                    <option value="<?= $t->id ?>" <?= ($car && $car->transmission_id==$t->id) ? 'selected' : '' ?>>
                        <?= $t->name ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <input type="text" name="mileage" class="form-control" value="<?= $car ? html_escape($car->mileage) : '' ?>"
            ...
            <div class="mb-3">
                ...
                <select name="city_id" class="form-select" required>
                    ...
                    <option value="<?= $c->id ?>" <?= ($car && $car->city_id==$c->id) ? 'selected' : '' ?>><?= $c->name ?>
                    </option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div class="col-md-6">
            ...
            <div class="mb-3">
                <div class="form-check"><input type="checkbox" name="is_featured" class="form-check-input" value="1"
                        <?= ($car && $car->is_featured) ? 'checked' : '' ?>><label class="form-check-label">Featured</label>
                </div>
            </div>
            <div class="mb-3">
    ...
    <button type="submit" name="submit" id="submit" class="btn btn-primary">Save Car</button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $('#brandId').change(function() {
            ...
            $.get('<?= base_url("admin/cars/models_by_brand") ?>?brand_id=' + bid, function(d) {
                ...
                $.each(d, function(i, m) {
                    o += '<option value="' + m.id + '">' + m.name + '</option>';
                });
                $('#modelId').html(o);
            });
        });
